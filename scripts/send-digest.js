...
function buildItemHtml(item, userId, ward) {
  ...
  return `
    <div style="background:#ffffff;border:1px solid #dce3ed;border-radius:8px;padding:20px;margin-bottom:16px;">
      ...
      <h3 style="margin:8px 0;font-size:18px;color:#1a1a2e;">${item.plain_title}</h3>
      <p style="color:#3d4f6f;font-size:14px;line-height:1.5;margin:8px 0;">${item.what_is_happening}</p>
      ...
      ${item.location ? `<p style="margin:8px 0;"><a href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(item.location + ', Jersey City, NJ')}" style="font-size:13px;color:#1e3a5f;text-decoration:none;">&#x1F4CD; ${item.location}</a></p>` : ''}
      ${deadlineHtml}
      ${item.source_url || item.meeting_url ? `<p style="margin:8px 0;">${item.source_url ? `<a href="${item.source_url}" style="font-size:13px;color:#1e3a5f;text-decoration:none;">View full document</a>` : ''}${item.source_url && item.meeting_url ? ' &middot; ' : ''}${item.meeting_url ? `<a href="${item.meeting_url}" style="font-size:13px;color:#1e3a5f;text-decoration:none;">Meeting agenda</a>` : ''}</p>` : ''}
...
function buildNoticeHtml(notice) {
  ...
  return `
    <div style="padding:10px 0;border-bottom:1px solid #f3f4f6;">
      ...
      ${notice.location ? `<p style="margin:4px 0 0;"><a href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(notice.location + ', Jersey City, NJ')}" style="font-size:12px;color:#1e3a5f;text-decoration:none;">&#x1F4CD; ${notice.location}</a></p>` : ''}
    </div>`;
}
...
function buildDevelopmentHtml(item, userId, ward) {
  ...
  return `
    <div style="background:#ffffff;border:1px solid #dce3ed;border-radius:8px;padding:16px;margin-bottom:12px;">
      ...
      <h3 style="margin:6px 0;font-size:16px;color:#1a1a2e;">${item.plain_title}</h3>
      <div style="background:#fafafa;border-radius:6px;padding:8px 10px;margin:8px 0;display:flex;align-items:center;gap:8px;">
        <span style="font-size:18px;">${icon}</span>
        <span style="font-size:13px;color:#374151;">${item.personal_impact}</span>
      </div>
      ${item.location ? `<p style="margin:6px 0 0;"><a href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(item.location + ', Jersey City, NJ')}" style="font-size:12px;color:#1e3a5f;text-decoration:none;">&#x1F4CD; ${item.location}</a></p>` : ''}
      <div style="margin-top:10px;">
        <a href="${repUrl}" style="display:inline-block;background:#d4a843;color:#ffffff;padding:6px 14px;border-radius:6px;text-decoration:none;font-size:13px;font-weight:600;border:1px solid #c09535;">&#x1F4E8; Let your rep know</a>
      </div>
    </div>`;
}
...
function buildDigestHtml(profile, items, weekDate, devNotices = [], notices = []) {
  const itemsHtml = items.length > 0
    ? `<h2 style="font-size:18px;color:#1a1a2e;margin:0 0 12px;padding-bottom:8px;border-bottom:2px solid #dce3ed;">\u{1F3DB}\uFE0F Ordinances &amp; Resolutions</h2>
  ...
  const devHtml = devNotices.length > 0
    ...
    <div style="margin-top:24px;">
      <h2 style="font-size:18px;color:#1a1a2e;margin:0 0 12px;padding-bottom:8px;border-bottom:2px solid #dce3ed;">\u{1F3D7}\uFE0F Development &amp; Zoning</h2>
  ...
  const noticesHtml = notices.length > 0
    ? `
    <div style="margin-top:24px;">
      <h2 style="font-size:18px;color:#1a1a2e;margin:0 0 12px;padding-bottom:8px;border-bottom:2px solid #dce3ed;">Community Notices</h2>
      <div style="background:#ffffff;border:1px solid #dce3ed;border-radius:8px;padding:12px 16px;">
...
<body style="margin:0;padding:0;background:#f5f7fa;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <div style="max-width:600px;margin:0 auto;padding:20px;">
    <div style="text-align:center;padding:24px 0;">
      <h1 style="margin:0;font-size:24px;color:#1a1a2e;">Your MyBloc Digest</h1>
    ...
    <div style="text-align:center;padding:24px 0;border-top:1px solid #dce3ed;margin-top:24px;">
...
function buildWelcomeHtml(profile, items, weekDate) {
  ...
  const repHtml = rep ? `
    <div style="background:#f0f4f8;border:1px solid #c8d6e5;border-radius:8px;padding:16px;margin-bottom:20px;">
      <p style="margin:0 0 6px;font-size:14px;font-weight:600;color:#1e3a5f;">Your council rep: ${rep.name} (Ward ${profile.ward})</p>
      <p style="margin:0 0 6px;font-size:13px;color:#1e3a5f;">Email: <a href="mailto:${rep.email}" style="color:#1e3a5f;">${rep.email}</a></p>
      <p style="margin:0;font-size:13px;color:#374151;">Your personalized digest arrives every Thursday.</p>
    </div>` : '';

  const previewHtml = hasItems ? `
    <p style="font-size:14px;color:#3d4f6f;margin-bottom:16px;">Here are a few items from the latest council agenda that may affect you:</p>
    ${itemsHtml}` : `
    <p style="font-size:14px;color:#3d4f6f;margin-bottom:16px;">Your first personalized digest will arrive this Thursday with the latest from the Jersey City Council — tailored to your ward and interests.</p>`;
...
<body style="margin:0;padding:0;background:#f5f7fa;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <div style="max-width:600px;margin:0 auto;padding:20px;">
    <div style="text-align:center;padding:24px 0;">
      <h1 style="margin:0;font-size:24px;color:#1a1a2e;">Welcome to MyBloc</h1>
    ...
    <div style="text-align:center;margin:24px 0;">
      <a href="${VOTE_BASE_URL}/manage.html?user=${profile.id}" style="display:inline-block;padding:12px 28px;background:#d4a843;color:#fff;border-radius:8px;text-decoration:none;font-size:14px;font-weight:600;">Update your profile</a>
    </div>
